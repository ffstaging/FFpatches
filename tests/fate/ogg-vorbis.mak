...
FATE_OGG_COPY_VORBIS += fate-ogg-copy-vorbis-chained-meta
fate-ogg-copy-vorbis-chained-meta: $(APITESTSDIR)/api-dump-stream-meta-test$(EXESUF)
fate-ogg-copy-vorbis-chained-meta: REF = $(SRC_PATH)/tests/ref/fate/ogg-vorbis-chained-meta.txt
fate-ogg-copy-vorbis-chained-meta: CMD = run_with_temp "$(FFMPEG) -nostdin -hide_banner -loglevel quiet -i $(TARGET_SAMPLES)/ogg-vorbis/chained-meta.ogg -c copy -f ogg -y" "$(APITESTSDIR)/api-dump-stream-meta-test$(EXESUF)" ogg

FATE_SAMPLES_FFMPEG += $(FATE_OGG_COPY_VORBIS-yes)

fate-ogg-copy-vorbis: $(FATE_OGG_COPY_VORBIS)
