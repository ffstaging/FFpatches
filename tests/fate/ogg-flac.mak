...
FATE_OGG_COPY_FLAC += fate-ogg-copy-flac-chained-meta
fate-ogg-copy-flac-chained-meta: $(APITESTSDIR)/api-dump-stream-meta-test$(EXESUF)
fate-ogg-copy-flac-chained-meta: REF = $(SRC_PATH)/tests/ref/fate/ogg-flac-chained-meta.txt
fate-ogg-copy-flac-chained-meta: CMD = run_with_temp "$(FFMPEG) -nostdin -hide_banner -loglevel quiet -i $(TARGET_SAMPLES)/ogg-flac/chained-meta.ogg -c copy -f ogg -y" "$(APITESTSDIR)/api-dump-stream-meta-test$(EXESUF)" ogg

FATE_SAMPLES_FFMPEG += $(FATE_OGG_COPY_FLAC-yes)

fate-ogg-copy-flac: $(FATE_OGG_COPY_FLAC)
