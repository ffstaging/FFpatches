...
{
    LOCAL_ALIGNED_32(uint8_t, in, [HEIGHT * WIDTH_PADDED]);
    ...
    memset(in, 0, HEIGHT * WIDTH_PADDED);
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++)
            in[y * WIDTH_PADDED + x] = rnd() & 0xFF;
    }
    ...
    if (depth == 16)
        w /= 2;
    w -= 8; /* ensure odd tail is handled correctly */

    if (check_func(ff_blackdetect_get_fn(depth), "blackdetect%d", depth)) {
        unsigned count_ref = call_ref(in, line_size, w, HEIGHT, threshold);
        unsigned count_new = call_new(in, line_size, w, HEIGHT, threshold);
        if (count_ref != count_new) {
            fprintf(stderr, "blackdetect%d: count mismatch: %u != %u\n",
                    depth, count_ref, count_new);
            fail();
        }
        bench_new(in, line_size, w, HEIGHT, 16);
    }
}
