...
typedef struct HTTPContext {
    ...
    /* Minimum and maximum request size (for partial requests). */
    uint64_t request_size_min;
    uint64_t request_size_max;
    uint64_t request_size; ///< current request size, resets on seek
    uint8_t *post_data;
    int post_datalen;
    int is_akamai;
    int is_mediagateway;
...
static const AVOption options[] = {
    ...
    { "request_size_min", "minimum size (in bytes) of each request", OFFSET(request_size_min), AV_OPT_TYPE_INT64, { .i64 = 1 << 15   }, 1024, INT64_MAX, D },
    { "request_size_max", "maximum size (in bytes) of each request", OFFSET(request_size_max), AV_OPT_TYPE_INT64, { .i64 = INT64_MAX }, 1024, INT64_MAX, D },
                     ...
                     AVDictionary **options)
{
    ...
    if (s->multiple_requests && s->seekable != 0) {
        if (s->request_size_max < s->request_size_min) {
            av_log(h, AV_LOG_ERROR,
                   "Invalid request size range: min %"PRIu64" > max %"PRIu64"\n",
                   s->request_size_min, s->request_size_max);
            return AVERROR(EINVAL);
        }

        s->request_size = s->request_size_min;
    }
...
static int http_read_header(URLContext *h)
{
    ...
    if (s->willclose || h->is_streamed)
        s->request_size = 0; /* no point in using multiple requests */
                        ...
                        const char *proxyauth)
{
    ...
    if (!has_header(s->headers, "\r\nRange: ") && !post &&
        (s->off > 0 || s->end_off || s->request_size || s->seekable != 0))
    {
        av_bprintf(&request, "Range: bytes=%"PRIu64"-", s->off);
        if (s->request_size) {
            uint64_t target_off = s->off + s->request_size;
    ...
    err = (off == s->off) ? 0 : -1;
    if (!err && s->request_size) {
        /* double request size after successful request */
        uint64_t next_size = s->request_size << 1;
        if (next_size < s->request_size) /* overflow */
            next_size = UINT64_MAX;
        s->request_size = FFMIN(next_size, s->request_size_max);
    }
...
static int64_t http_seek_internal(URLContext *h, int64_t off, int whence, int force_reconnect)
{
    ...
    if (s->request_size) /* reset request size on seek */
        s->request_size = s->request_size_min;
    ...
    if (!s->willclose && s->range_end && remaining <= s->short_seek_size) {
