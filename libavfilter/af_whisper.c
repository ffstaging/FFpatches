...
static void run_transcription(AVFilterContext *ctx, AVDictionary **metadata, int frames)
{
    WhisperContext *wctx = ctx->priv;
    frames = FFMAX(0, FFMIN(frames, wctx->audio_buffer_fill_size));

    if (!wctx->ctx_wsp || frames == 0)
        return;

    float duration = (float) frames / WHISPER_SAMPLE_RATE;

    av_log(ctx, AV_LOG_INFO,
           "run transcription %d/%d samples (%.2f seconds)...\n", frames, wctx->audio_buffer_fill_size, duration);
    ...
    if (whisper_full(wctx->ctx_wsp, params, wctx->audio_buffer, frames) != 0) {
        av_log(ctx, AV_LOG_ERROR, "Failed to process audio with whisper.cpp\n");
        return;
    }

    const int64_t timestamp = wctx->frames * 1000 / WHISPER_SAMPLE_RATE;
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        av_log(ctx, AV_LOG_INFO, "  [%ld-%ld%s]: \"%s\"\n",
               timestamp + t0, timestamp + t1, turn ? " (turn)" : "", text_cleaned);
        ...
        if (wctx->avio_context) {
            const int64_t start_t = timestamp + t0;
            const int64_t end_t = timestamp + t1;
            char *buf = NULL;
    ...
    if (wctx->audio_buffer_fill_size > frames)
        memcpy(wctx->audio_buffer, wctx->audio_buffer + frames,
               (wctx->audio_buffer_fill_size - frames) * sizeof(*wctx->audio_buffer));
    wctx->audio_buffer_fill_size -= frames;
