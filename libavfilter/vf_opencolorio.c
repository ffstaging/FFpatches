...
static av_cold int init(AVFilterContext *ctx) {
  ...
  if (s->out_format_string != NULL) {
    ...
    // Default to same as input format (see later).
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame) {
  ...
  int channels = desc->nb_components; // 3 or 4
  s->channels = channels;
...
static const AVOption ocio_options[] = {
    ...
    { "display", "Output display, used insted of output color space.", OFFSET(display), AV_OPT_TYPE_STRING, {.str=NULL}, 0, 0, FLAGS },
