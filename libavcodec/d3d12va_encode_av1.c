...
typedef struct D3D12VAEncodeAV1Context {
    ...
    // Writer structures.
    FFHWBaseEncodeAV1 units;
    FFHWBaseEncodeAV1Opts unit_opts;
...
static int d3d12va_encode_av1_init_sequence_params(AVCodecContext *avctx)
{
    ...
    err = ff_hw_base_encode_init_params_av1(base_ctx, avctx,
...
static int d3d12va_encode_av1_configure(AVCodecContext *avctx)
{
    ...
    if (ctx->rc.Mode == D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_CQP) {
        ...
        fixed_qp_inter = av_clip(ctx->rc_quality, 0, 255);

        if (avctx->i_quant_factor > 0.0)
            fixed_qp_key = av_clip((avctx->i_quant_factor * fixed_qp_inter +
                                    avctx->i_quant_offset) + 0.5, 0, 255);
        else
...
static const D3D12VAEncodeType d3d12va_encode_type_av1 = {
    ...
    .write_picture_header  = &d3d12va_encode_av1_write_picture_header,

    .write_tile_group      = &d3d12va_encode_av1_write_tile_group,
};
