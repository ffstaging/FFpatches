...
#define RW(N, ...) (SwsReadWriteOp) { .elems = N, __VA_ARGS__ }
...
static FmtInfo fmt_info_irregular(enum AVPixelFormat fmt)
{
    ...
    case AV_PIX_FMT_MONOBLACK:
        return (FmtInfo) { RW(1, .frac = 3), RGBA };
    case AV_PIX_FMT_RGB4:
        return (FmtInfo) { RW(1, .frac = 1), RGBA, {{ 1, 2, 1 }} };
    case AV_PIX_FMT_BGR4:
        return (FmtInfo) { RW(1, .frac = 1), BGRA, {{ 1, 2, 1 }} };

    /* Sub-packed 8-bit aligned formats */
    case AV_PIX_FMT_RGB4_BYTE:  return (FmtInfo) { RW(1), RGBA, {{ 1, 2, 1 }} };
    case AV_PIX_FMT_BGR4_BYTE:  return (FmtInfo) { RW(1), BGRA, {{ 1, 2, 1 }} };
    case AV_PIX_FMT_RGB8:       return (FmtInfo) { RW(1), RGBA, {{ 3, 3, 2 }} };
    case AV_PIX_FMT_BGR8:       return (FmtInfo) { RW(1), BGRA, {{ 2, 3, 3 }} };

    /* Sub-packed 16-bit aligned formats */
    case AV_PIX_FMT_RGB565LE:
    case AV_PIX_FMT_RGB565BE:
        return (FmtInfo) { RW(1), RGBA, {{ 5, 6, 5 }} };
    case AV_PIX_FMT_BGR565LE:
    case AV_PIX_FMT_BGR565BE:
        return (FmtInfo) { RW(1), BGRA, {{ 5, 6, 5 }} };
    case AV_PIX_FMT_RGB555LE:
    case AV_PIX_FMT_RGB555BE:
        return (FmtInfo) { RW(1), RGBA, {{ 5, 5, 5 }} };
    case AV_PIX_FMT_BGR555LE:
    case AV_PIX_FMT_BGR555BE:
        return (FmtInfo) { RW(1), BGRA, {{ 5, 5, 5 }} };
    case AV_PIX_FMT_RGB444LE:
    case AV_PIX_FMT_RGB444BE:
        return (FmtInfo) { RW(1), RGBA, {{ 4, 4, 4 }} };
    case AV_PIX_FMT_BGR444LE:
    case AV_PIX_FMT_BGR444BE:
        return (FmtInfo) { RW(1), BGRA, {{ 4, 4, 4 }} };

    /* Sub-packed 32-bit aligned formats */
    case AV_PIX_FMT_X2RGB10LE:
    case AV_PIX_FMT_X2RGB10BE:
        return (FmtInfo) { RW(1), ARGB, {{ 2, 10, 10, 10 }} };
    case AV_PIX_FMT_X2BGR10LE:
    case AV_PIX_FMT_X2BGR10BE:
        return (FmtInfo) { RW(1), ABGR, {{ 2, 10, 10, 10 }} };
    case AV_PIX_FMT_XV30LE:
    case AV_PIX_FMT_XV30BE:
        return (FmtInfo) { RW(1), AVYU, {{ 2, 10, 10, 10 }} };
    case AV_PIX_FMT_V30XLE:
    case AV_PIX_FMT_V30XBE:
        return (FmtInfo) { RW(1), VYUA, {{ 10, 10, 10, 2 }} };
    /* 3-component formats with extra padding */
    case AV_PIX_FMT_RGB0:   return (FmtInfo) { RW(4), RGBA };
    case AV_PIX_FMT_BGR0:   return (FmtInfo) { RW(4), BGRA };
    case AV_PIX_FMT_0RGB:   return (FmtInfo) { RW(4), ARGB };
    case AV_PIX_FMT_0BGR:   return (FmtInfo) { RW(4), ABGR };
    case AV_PIX_FMT_VUYX:   return (FmtInfo) { RW(4), VUYA };
    case AV_PIX_FMT_XV36LE:
    case AV_PIX_FMT_XV36BE:
        return (FmtInfo) { RW(4), UYVA, .shift = 4 };
    case AV_PIX_FMT_XV48LE:
    case AV_PIX_FMT_XV48BE:
        return (FmtInfo) { RW(4), UYVA };
    }
...
static int fmt_analyze_regular(enum AVPixelFormat fmt, SwsReadWriteOp *rw_op,
                       ...
                       SwsPixelType *pixel_type, SwsPixelType *raw_type)
{
    ...
    *pixel_type = *raw_type = fmt_pixel_type(fmt);
    ...
    if (is_regular_fmt(fmt)) {
        *pack_op = (SwsPackOp) {0};
        return fmt_analyze_regular(fmt, rw_op, swizzle, shift);
    }

    FmtInfo info = fmt_info_irregular(fmt);
    if (!info.swizzle.mask)
        return AVERROR(ENOTSUP);
    ...
    *shift   = info.shift;
    rw_op->packed = desc->nb_components > 1;
