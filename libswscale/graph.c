...
static SwsFrame get_field(SwsGraph *graph, const AVFrame *avframe)
{
    SwsFrame frame;
...
void ff_sws_graph_run(SwsGraph *graph, const AVFrame *dst, const AVFrame *src)
{
    ...
    const SwsFrame src_field = get_field(graph, src);
    const SwsFrame dst_field = get_field(graph, dst);
